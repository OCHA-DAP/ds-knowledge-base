...
|---|---|---|---|
| NHC Pipeline (bundle) | `dbx:959161297191654` | `0 0,30 0/3 * * ?` UTC — every 3h; the `:30` run is a WSP late-arrival fill (stages short-circuit on an already-present `issued_time`) | live, **`mode=prod`** — cut over from `mode=dev` 2026-09-23 (see gotchas) |
| GDACS/ADAM Pipeline (bundle) | `dbx:197203772269744` | `0 0 0/3 * * ?` UTC — every 3h, matches NHC cadence | live, **`mode=prod`** — cut over from `mode=dev` 2026-09-23 |
...
| Run IBTrACS (bundle) | `dbx:737451582204703` | daily, `27 0 16 * * ?` UTC | live, **`mode=prod`** (via the bundle's `etl_mode` variable — deliberately NOT part of the NHC dev-cutover, which itself flipped to prod 2026-09-23); full-archive ETL (`--dataset-type ALL`) on DS4_v2, ~70–90 min; `on_failure` → tristan.downing@un.org. Green since 2026-09-16 (first successes ever for this ETL). |
...
- **Data-plane cutover — `nhc_pipeline`/`gdacs_adam_pipeline` flipped `mode: dev` → `prod` (observed 2026-09-23, [#653](https://github.com/OCHA-DAP/ds-knowledge-base/issues/653)).** Until then `databricks.yml`'s `prod` target set `variables.mode: dev` with a `TEMPORARY` comment ("flip to prod once we're ready to write prod tables"), so these jobs wrote the DEV DB and prod `storms.*` rows weren't refreshed by them. The deployed jobs now carry `mode=prod` (detected by `check_infra_drift.py`), so prod `storms.*` rows from here on come from these jobs; **pre-2026-09-23 prod rows may be stale/gappy** and the DEV DB stops being the freshest copy — check which stage any consumer reads. <!-- TODO: confirm with the ds-storms-pipeline owner that the databricks.yml TEMPORARY comment is gone, whether prod was backfilled, and whether dev keeps being written (e.g. by the dev target). -->
...
- [storms-alerts](storms-alerts.md) — email alerting off this pipeline's exposure tables (runs on its own job, `Storm Alert` `dbx:500881901438881` — reading `stage=prod` since the 2026-09-23 cutover — on ephemeral Job Compute since 2026-09-15, previously pinned to an interactive cluster).
