...
- **SEAS5 `valid_date` is the END of the aggregation window.** ECMWF's `valid_date` attribute marks the end of the monthly aggregation window, while our COG metadata treats `valid_time` as the **start** — the two conventions differ by one month and are easy to misread. See [ds-raster-pipelines#9](https://github.com/OCHA-DAP/ds-raster-pipelines/issues/9) and `examples/ecmwf_temporal_vars.md` in that repo.
