---
content_type: framework-external
framework: ifrc-nga-flood
org: IFRC
country_iso3: NGA
hazard: flood
status: active
valid_until: null
trigger_summary: null
data_sources: []
prearranged_funding_usd: 249716
funding_by_source: {}
target_people: 7500
framework_doc: /download/file-2985
framework_doc_date: null
sources:
- https://www.anticipation-hub.org/experience/global-map
activations: []
last_checked: '2026-07-10'
extra:
  hub_stub: true
  hub_captions:
  - '2022: Flood (IFRC) [IFRC] [Red Cross Society of Niger]'
  - '2023: Flood (IFRC) [Nigerian Red Cross Society]'
  - '2024: Flood (IFRC) [Nigerian Red Cross Society]'
  hub_years:
  - '2022'
  - '2023'
  - '2024'
  implementing:
  - Nigerian Red Cross Society
visibility: public
---

# IFRC — Nigeria flood

**Stub** — created from the [Anticipation Hub global map](https://www.anticipation-hub.org/experience/global-map) inventory
(2026-07-10); core facts only, pending enrichment (trigger, funding detail, activations —
see `external-frameworks/README.md`). Hub listing: 2022: Flood (IFRC) [IFRC] [Red Cross Society of Niger]; 2023: Flood (IFRC) [Nigerian Red Cross Society]; 2024: Flood (IFRC) [Nigerian Red Cross Society].

## Sources
- [Anticipation Hub global map](https://www.anticipation-hub.org/experience/global-map) (inventory record, fetched 2026-07-10)
- [Framework document](/download/file-2985)
