...
valid_until: null
trigger_summary: >-
  Uganda Red Cross Society (URCS) activates when GloFAS forecasts ≥60% probability of a
  5-year-return-period flood in flood-prone districts, expected to affect more than 1,000
  households, at a 5-day lead time with a false-alarm ratio of ≤0.5 — triangulated against
  Uganda National Meteorological Authority (UNMA) and ICPAC forecasts.
...
extra:
  ...
  scope_detail: >-
    16 potentially exposed districts nationally identified during EAP design; Ntoroko
    (Western, 36,590 exposed) and Buyende (Eastern, 7,732 exposed) had the highest exposed
    populations. The 2023 activation itself covered only Butaleja, Kikuube and Ntoroko.
  funding_chf: >-
  ...
  schema_strain: >-
    ...
    instead. No `valid_until` found (EAP appears open-ended pending a future generation).
    No OCHA/CERF-owned Uganda flood framework exists in this KB, so this is not a
    component of a collective framework — it is URCS's own EAP.
visibility: public
---
...
targets flood-prone districts nationally (16 identified as exposed at design time) and has
activated once to date, in November 2023.

## Trigger
GloFAS forecasts of ≥60% probability that a river will exceed its 5-year-return-period
flood level in flood-prone districts, expected to affect more than 1,000 households, at a
5-day lead time, with a false-alarm ratio capped at 0.5. The GloFAS signal is triangulated
against forecasts from the Uganda National Meteorological Authority (UNMA) and ICPAC.

## Funding & scope
CHF 348,761 (≈US$395,000 / €362,000) pre-arranged through the IFRC DREF. Exposure mapping
during EAP design flagged 16 potentially affected districts, with Ntoroko (36,590 exposed)
and Buyende (7,732 exposed) the highest. Early actions span shelter, WASH, cash & voucher
assistance and disaster-risk-reduction (evacuation mapping, awareness campaigns).
