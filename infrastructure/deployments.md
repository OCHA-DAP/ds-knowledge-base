...
| Somalia flood-trigger site (landing + `/trigger/` report) | `ds-aa-som-floods` | GH Pages (workflow mode, `deploy-pages.yml`; `pages/` → root; **private repo** → org-members-only `*.pages.github.io` subdomain) | landing page ([convention](../methods/static-data-apps.md#one-repo-one-pages-site--the-landing-page-convention)) + rendered trigger report | https://sturdy-memory-v6voe4m.pages.github.io/ | [analysis/som-flooding-multisource](../analysis/som-flooding-multisource.md) |
