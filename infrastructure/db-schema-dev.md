...
**9 schemas · 69 tables · 26.5 GB total.**
...
| `plan_caseloads` | 3.3k | 600.0 KB | <details><summary>11 cols</summary>plan_id `integer`, entity_id `integer`, cluster_name `text`, requirements `bigint`, total_population `bigint`, in_need `bigint`, targeted `bigint`, affected `bigint`, expected_reach `bigint`, reached `bigint`, refreshed_at `timestamp with time zone`</details> |
...
## `ipc` — 3 tables · 303.3 MB

| table | rows (est) | size | columns |
|---|--:|--:|---|
| `population` | 507.6k | 178.7 MB | <details><summary>14 cols</summary>iso3 `text`, level `text`, level1_name `text`, area_name `text`, analysis_date `date`, period_type `text`, reference_period_start `date`, reference_period_end `date`, phase `text`, population `bigint`, fraction `double precision`, country_population `bigint`, source_dataset `text`, refreshed_at `timestamp with time zone`</details> |
| `population_admin` | 353.9k | 124.4 MB | <details><summary>15 cols</summary>location_code `text`, location_name `text`, admin1_code `text`, admin1_name `text`, admin2_code `text`, admin2_name `text`, admin_level `integer`, ipc_phase `text`, ipc_type `text`, population_in_phase `bigint`, population_fraction_in_phase `double precision`, reference_period_start `date`, reference_period_end `date`, resource_hdx_id `text`, refreshed_at `timestamp with time zone`</details> |
| `analyses` | 544 | 296.0 KB | <details><summary>9 cols</summary>analysis_id `text`, title `text`, link `text`, country_iso2 `text`, year `integer`, condition `text`, created `date`, modified `date`, refreshed_at `timestamp with time zone`</details> |

## `kb_usage` — 1 tables · 168.0 KB

| table | rows (est) | size | columns |
|---|--:|--:|---|
| `events` | 315 | 168.0 KB | <details><summary>11 cols</summary>id `bigint`, ts `timestamp with time zone`, tier `text`, tool `text`, arg_summary `text`, ok `boolean`, empty `boolean`, result_chars `integer`, latency_ms `integer`, error `text`, session `text`</details> |
...
| `nhc_wsp_exposure` | 595.8k | 110.4 MB | <details><summary>8 cols</summary>issued_time `timestamp without time zone`, wind_threshold_kt `smallint`, percentage `smallint`, atcf_id `character varying`, admin_level `smallint`, iso3 `character varying`, pcode `character varying`, pop_exposed `integer`</details> |
...
| `storm_id_lookup` | 334 | 144.0 KB | <details><summary>5 cols</summary>gdacs_eventid `integer`, atcf_id `character varying`, sid `character varying`, adam_eventid `integer`, last_updated `timestamp without time zone`</details> |
