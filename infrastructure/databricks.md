...
- **⚠️ A scheduled prod job pinned to a durable interactive cluster is fragile.** If that cluster is deleted/renamed or its owner leaves, the job breaks — prod jobs should target Job Compute, not a standing interactive cluster. **`Storm Alert` (job `500881901438881`) was moved off `0515-…` onto ephemeral Job Compute on 2026-09-15** ([#620](https://github.com/OCHA-DAP/ds-knowledge-base/issues/620); the registry reads `compute=job-compute`, which `gen_pipeline_registry.py` emits only when the job's `new_cluster.policy_id` is the Job Compute policy `000C79D951EAF0D6` — so this is a real redeploy, **not** the read-side flap described below, which can only swap `personal:` ↔ `existing:`). That leaves the `[dev adm_tdowning] NHC Pipeline` (job `583285176982712`) as the remaining job on the **`existing_cluster_id` `0515-…`** — a dev job, so lower stakes. _(infra-drift **flap — it is a read-side artifact, not a cluster change.** `personal:<id>` is assigned only when the id appears in `clusters list` with `cluster_source` `UI`/`API`; an identity that can't list clusters gets an **empty** personal set, and both jobs silently reclassify to `existing:<id>` (and the `PERSONAL-CLUSTER` warn can't fire). Confirmed for **2026-08-10 → 2026-08-11**: the 08-10 fingerprint was taken with the org-scoped `DSCI_DATABRICKS_TOKEN`, which **lacks the `clusters` scope** ([automation.md](automation.md)), and flipped straight back to `personal:` on the next locally-authenticated run. The earlier blips (2026-07-21, 2026-07-26 — each reverting the next run, 07-23 / 07-27) have the same signature; the cause of those empty reads is **unconfirmed**. **Longest run so far: 2026-08-12 → 2026-08-15** (two consecutive fingerprints read `existing:`), reverting to `personal:` on **2026-08-27** ([#573](https://github.com/OCHA-DAP/ds-knowledge-base/issues/573)) — so "it reverts on the very next run" is not a rule; don't read a multi-day `existing:` streak as a real re-pinning. Read an empty personal-cluster column as *not checked*, not *not personal*; to actually settle it, `databricks clusters get 0515-161935-i2w5mxhc`. The pinning fragility stands either way.)_
...
- **The generic wrapper `databricks/run_task.py`** (floodexposure / tcd / nga / the four mirrors / hdx-floodscan): every task is `run_task.py <script> --stage {dev|prod} [--env K=V] [--secret NAME] [--optional-secret NAME] [-- script args]`. It sets `STAGE`, copies `src/` + `scripts|pipelines/` off the wsfs checkout to `/local_disk0` (importing off the FUSE mount is flaky), resolves extra `dsci` secrets with `dbutils.secrets.get` at run time and shells out. **Extra secrets go through `--secret`, not `spark_env_vars: {{secrets/dsci/NAME}}`** — a missing or misnamed key in `spark_env_vars` stops the job cluster from launching with an opaque error, while `--secret` fails the task with a readable message (`--optional-secret` for keys the script can do without, e.g. `IPC_AUTH`).
- **Pages sites that used to query the DB from a GitHub runner** (the mirrors, since the private-endpoint cutover) use the *site-data-via-blob* pattern: the Databricks job runs the unchanged `export_site_data.py` then `scripts/site_data_blob.py upload` → dev blob `projects/<repo>/site-data/`; `deploy-site.yml` runs `download` with the org `DSCI_AZ_BLOB_DEV_SAS` on its old backstop cron and deploys the same `site/` dir. Output is identical by construction. Gotcha: the dev storage account has hierarchical namespace, so every folder is a zero-byte marker blob (`metadata.hdi_isfolder`) — list/download/stale-delete must skip them. Alternative when the build needs SQL: the ds-aa-tracking snapshot → throwaway Postgres model.
