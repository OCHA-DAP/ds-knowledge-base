...
**They can mismatch during a cutover:** until 2026-09-23 the live `NHC Pipeline` and `GDACS/ADAM Pipeline` (and the downstream `Storm Alert`) deployed to **prod compute** but ran **`mode=dev`** (writing/reading the DEV DB) per a deliberate "flip to prod once ready" step in `ds-storms-pipeline/databricks.yml`; all three flipped to `mode=prod` on 2026-09-23 ([#653](https://github.com/OCHA-DAP/ds-knowledge-base/issues/653)). So "is this job producing prod data?" = **(prod target) AND (mode=prod)** — check both. The registry records both columns for exactly this reason.
...
- **DONE 2026-09-23 — NHC/GDACS cutover** (`mode: dev`→`prod` on the bundle jobs, plus `Storm Alert` → `stage=prod`; [#653](https://github.com/OCHA-DAP/ds-knowledge-base/issues/653)). Still open: **un-pause or retire `Run NHC`**, and reconcile with the **GHA `ds-nhc-forecast`** pipeline — the DAB `NHC Pipeline` now also writes prod `storms.nhc_*`, so the overlapping NHC writers (paused prod / now-prod DAB / GHA) are a concurrency + clarity hazard (see [nhc-forecast.md](../pipelines/nhc-forecast.md)).
- **Move `[dev adm_tdowning] NHC Pipeline` off the standing interactive cluster `0515-…`** onto Job Compute (`Storm Alert` done 2026-09-15, #620).
