...
- **⚠️ A scheduled prod job pinned to a durable interactive cluster is fragile.** `Storm Alert` (job `500881901438881`) and the `[dev adm_tdowning] NHC Pipeline` (job `583285176982712`) both run on the **`existing_cluster_id` `0515-…`** — if that cluster is deleted/renamed or its owner leaves, the jobs break. Prod jobs should target Job Compute, not a standing interactive cluster. _(infra-drift **flap**: on 2026-07-21 this cluster briefly reclassified as a **shared `existing`** cluster — `cluster_source` no longer `UI`/`API` — then on **2026-07-23 it flipped back to `personal`** (`cluster_source` `UI`/`API` again), so the registry's `PERSONAL-CLUSTER` flag fires again on the next regen. The oscillation itself is unexplained — whether the cluster was momentarily re-sourced/converted or this is a transient read is **unconfirmed**; verify against `databricks clusters get 0515-161935-i2w5mxhc`. The pinning fragility stands either way.)_
