...
graph LR
  ...
  class n_moz_cholera,n_moz_cyclones,n_cub_hurricanes,n_mmr_cyclones,n_hti_hurricanes,n_fji_storms framework;
  class n_fms_tc_outlook,n_mdg_monitoring,n_floodscan_ingest,n_storms_pipeline,n_storms_alerts,n_flood_gfm,n_moz_cholera_monitoring,n_moz_cyclones_monitoring,n_hti_hurricanes_monitoring,n_glb_cyclones_impactmodel,n_hurricanes_monitoring,n_imerg,n_floodexposure_monitoring,n_glb_tropicalcyclones pipeline;
  class n_hti_hurricanes_app,n_floodexposure_monitoring_app,n_seas5_viz,n_fji_storms_app,n_seas5_skill app;
  class n_public_imerg,n_app_floodscan_exposure_regions,n_storms_nhc_tracks_geo,n_storms_ibtracs_storms,n_app_admin_lookup,n_app_quantile,n_app_adm,n_app_quantile_regions,n_public_era5,n_app_floodscan_exposure,n_storms_nhc_storms,n_storms_ibtracs_tracks_geo,n_public_polygon,n_public_seas5 table;
  class n_aws_smtp,n_listmonk infra;
  class n_mfed_zips_,n_ibtracs,n_database,n_ghsl,n_gfm_stac external;
```
...
- **Frameworks with no declared edges (21):** their monitoring isn't yet ingested as a pipeline, or `depends_on` is unset — most run monitoring in-repo. Edges fill in as pipelines/apps are ingested.
