...
**`all_in` and "able to trigger" (capacity).** Separate from lifecycle *status*, the map derives whether a framework **can still fire now** — the **"able to trigger" ring** (solid green = able & in monitoring season this month; pale green = able & off-season; **no ring = cannot trigger**). A framework becomes **not able** after a full activation (its envelope is spent) — **except** cholera (recurring) and **multi-window SPLIT** frameworks. Set the **`all_in`** frontmatter field: **`true`/omitted** = one envelope released on any trigger (spent after firing → not able until renewed); **`false`** = each window has its own budget and fires independently (stays able after one window fires). Set `all_in: false` only for genuinely independent-budget multi-window frameworks. Logic: `gen_public_site.py` (`able_to_trigger` / `retriggerable`).
