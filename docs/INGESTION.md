...
**REAL vs SIMULATED activation history — never conflate them.** A framework version's page carries two different activation records, and they answer different questions. (1) **Real-life activations** (`activations:` frontmatter + the factual part of `## Historical activations`): AA actually fired / funds released — but each fired under whatever trigger version was live *at the time*. (2) **Simulated (backtested / would-have-activated) history**: the retrospective run of *this version's* trigger mechanism over the historical record, from the version's calibration analysis. Triggers are almost always revised between versions, so **the real-life history is NOT evidence of how the current version's trigger behaves** — a question like "how often would the current framework have activated historically?" must be answered from the simulated record, never from the real-life list or the doc's "activated N times" summary. When ingesting the simulated record: calibration analyses typically explore **multiple threshold/constraint variants** (different percentiles, muting rules, baselines) — identify **which variant the framework team designates as authoritative** (ask, or take the analysis's stated recommendation) and cite the exact artifact (chapter/table/pipeline target + branch), noting that other variants in the same analysis give different counts. (Origin: DRC cholera 2026 — the framework adopted the **L3** spatial constraint, so its authoritative simulated history is the L3 per-year table in ch08 §9.7 (total 22, ~2/yr), *not* the L2.5 variant the DS team recommended in the same chapter, and *not* the real-life "activated 6 times" count in the framework PDF. Three different numbers for three different questions.)
