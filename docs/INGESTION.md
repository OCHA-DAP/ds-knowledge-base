...
| framework-external | `external-frameworks/<org>/` | An AA framework **owned by another org** (IFRC EAPs, WFP, FAO, START…). Deliberately **loose**: common-core frontmatter only (⊂ the OCHA schema, D77), web sources, `last_checked` instead of drift bots. See the [folder README](../external-frameworks/README.md). | core frontmatter + short prose |
...
| dataset | `infrastructure/datasets/` | A **third-party** data source we consume regularly but don't produce (IPC/CH, HDX, HRP/HNRP, FEWS NET, EM-DAT, WorldPop, FAO ASI/VHI, GHSL) | reference: access/API + auth, resolution, license, loader (`code_ref`), used-by (see the folder README) |
...
**Sector & agency breakdowns.** Where the doc gives amounts, also break the pre-arranged funding down in `funding_by_sector` and `funding_by_agency` (USD ints). Use the **doc's own sector labels** (don't harmonise — `"Health & Nutrition"` combined stays combined; `"Multi-Sector/MPCA"` stays as written). These breakdowns are often **partial**: a doc may itemise only the CERF portion by agency while the CBPF/AHF portion is by sector with partners still TBD — record what's stated, keep TBD-partner envelopes out of `funding_by_agency`, and state the coverage in one line at `extra.funding_breakdown_note` (e.g. "by-agency covers the CERF $12M only; AHF $10M is by sector, partners selected at allocation"). Each dict should sum to the portion it covers — cross-check against `funding_by_source`. Beware PDF table extraction: the raw `.txt` often scrambles row alignment in budget tables, so verify each amount is attached to the right agency/sector (the column totals and the stated source totals are the check). `{}` = the doc genuinely gives no per-sector/per-agency amounts (common for docs that defer to CERF project documents). **Alternative-scenario frameworks** (PHL-style: per-scenario/per-region budgets where only ONE alternative draws per event): do NOT sum the alternatives into the dicts — the sum would overshoot the envelope. Leave the dicts `{}` and record the per-scenario breakdowns in `extra.funding_breakdown_note`.

**Finest-grain rows (`funding_rows`).** Most docs publish the budget as a joint table — *(window ×) source × agency × sector* cells — of which the two dicts are just the marginals. Record those cells verbatim in `funding_rows` (`[{window, source, agency, sector, amount_usd}]`), omitting any axis the doc doesn't split on. Three hard rules: (1) **additive** — summing rows along any axis must reproduce the corresponding dict over the portion it covers, so window-conditional envelopes (MRT: one WFP budget that is Food Security if window 1 fires, Nutrition if window 2) and alternative scenarios stay out of the rows; (2) **stated only** — never impute a split the doc doesn't give; analysis conventions (e.g. the default *5% readiness / 95% action* split for rapid-onset frameworks whose doc doesn't price the readiness window) live in the DB loader (`load_aa_performance.py`), which flags such rows `imputed`, never in the page. Window budgets mean **authorization to spend**, not disbursement — CERF typically *disburses* 100% of the envelope at the readiness trigger so agencies can move fast, while spend authorization is ~5% readiness / rest on action confirmation (unspent funds reimbursed). A doc saying "fully disbursed at the readiness trigger" is a cash-flow statement, NOT a readiness budget; (3) `window` names match the Trigger-windows table. The rows feed `aa.funding_breakdown` in the dev DB; per-window row sums are cross-checked against `aa.window.allocation_usd` at load time.
...
Every page is a card in a catalog over the raw sources. Mandatory (the required frontmatter fields on every page): `source_repo`, `source_sha` (drift anchor, Phase 5), `code_ref`, and the authority-ranked document links above (`framework_doc`, `model_report`, `apps`). The full per-content-type schema — every required field, the status vocabularies, completeness — is each folder's `_TEMPLATE.md`; new frameworks follow `frameworks/_TEMPLATE.md`.
