...
| framework-external | `external-frameworks/<org>/` | An AA framework **owned by another org** (IFRC EAPs, WFP, FAO, START…). Deliberately **loose**: common-core frontmatter only (⊂ the OCHA schema, D76), web sources, `last_checked` instead of drift bots. See the [folder README](../external-frameworks/README.md). | core frontmatter + short prose |
