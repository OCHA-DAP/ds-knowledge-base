...
### Task 3: Docs — USING.md section + DESIGN.md D96
...
git commit -m "docs: watching-the-plugins-work section + D96 decision entry"
```
...
- [ ] **Step 2:** `gh pr create` (title "kb-access: activity notices — see the plugins working"; body summarizes spec, links D96; note the plugin paths are code-ownered so the PR needs @t-downing/@zackarno review per the D85 ruleset).
