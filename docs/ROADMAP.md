...
- [ ] **Phase 7 — Documents: extracts + Google Drive.** Make document knowledge greppable, under the [PRIVACY.md](PRIVACY.md) classification (D44: public/internal by **source**; metadata public, content per-source).
  ...
  - [~] **7b · Drive manifest (public metadata).** Target = **only the DS team shared drive** (`0AGYkOFcloQuyUk9PVA`; NOT the data-storage drive; **exclude `General - All AA projects / Data`**) → `infrastructure/drive-index.md` (+ `.drive-index.json`): per-item folder-path, title, type, dates, size, link. Metadata only → **public** (PII stripped — no owner emails). `scripts/gen_drive_index.py` written (`--render-only` to re-render). **Done:** top-level **seed** (27 folders + key files) captured via the Claude connector + rendered. **Pending:** the full recursive crawl — the connector is interactive-only, so the script does it via the Drive API with a **read-only service-account** (the shared drive shared to it; `GOOGLE_APPLICATION_CREDENTIALS`). _Next: provision the SA creds → full crawl → 7c content extraction (private)._
...
- `infrastructure/drive-index.md` — public manifest of the DS team shared drive (metadata only; content is internal — see `docs/PRIVACY.md`). Generated by `scripts/gen_drive_index.py`; currently a top-level seed pending the full crawl (needs Drive read-only creds).
