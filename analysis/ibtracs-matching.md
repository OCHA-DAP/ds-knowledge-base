...
discrepancies:
  ...
  - "[superseded] Precursor to the [`cerf-supplement`](../pipelines/cerf-supplement.md) pipeline (`ocha-dap/ds-cerf-supplement`), which now does the same CERF-allocation → IBTrACS-sid matching as a fully automated, DB-backed system (daily GitHub Actions + Claude matcher, writing `aa.cerf_allocation_storm` + `aa.cerf_supplement`, keyed on `ApplicationCode`). Treat `cerf-supplement` as authoritative; this notebook is dormant/historical."
