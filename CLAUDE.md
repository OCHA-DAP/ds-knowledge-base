...
- `infrastructure/self-healing-pipelines.md` — **proposal, not built** (D97): the remediation half of the above — detect→diagnose→**propose a PR**, never auto-repair. Failure taxonomy, guardrails (AA trigger logic carved out entirely), verification tiers, and a worked example showing why the obvious auto-fix would have been wrong.
