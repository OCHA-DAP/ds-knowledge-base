...
- **[docs/PRIVACY.md](docs/PRIVACY.md)** — *public vs internal*: classification follows the source; public-source full-text → in-repo `raw/`. Google Drive **content** is internal (private store), but its **metadata/manifest is public** (`infrastructure/drive-index.md`) — the catalog of what exists, not the content. Read before ingesting any new source.
