...
- `claude/` — the **team Claude Code distribution**: `CLAUDE.dsci.md` (the always-loaded config kernel every team session imports from its local clone — supersedes `ds-claude-config`; CI-budgeted, so link to KB pages instead of inlining) and `skills/` (`ds-team:*` skills — blob-io, hdx-brand, kb-doctor — auto-shipped to every machine by the session-start pull). Gated by `scripts/check_claude_assets.py`; **merging here deploys to the whole team's next session.**
