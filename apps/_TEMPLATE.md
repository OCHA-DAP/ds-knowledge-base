...
surfaces: []       # OTHER published URLs of this repo beyond deployment.url (a landing page, sibling products under the same Pages site): [{url, kind, title}]. deployment.url itself counts as declared — don't repeat it. Auto-filled daily by gen_pages_registry.py. A partner-run tool/document the work relies on (not built by us) gets `origin: external` → the hub's External resources section (D104).
