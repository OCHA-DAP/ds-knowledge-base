...
extra:
  ...
  pipeline_run: "Databricks job 'SEAS5 Skill Monthly Refresh' (databricks.yml, 7th 03:00 UTC, Job Compute): compute_skill / _adm1 / _adm2 / _raster + monthly_clim + hdx-signal inputs -> blob stage=dev; since 2026-09-25 (the DBs are only reachable from the workspace)"
  gh_pages_rebuild: "automatic — the same job runs every exporter, verify_site_data.py --expect <YYYY-MM>, and uploads docs/data + docs/raster/data + docs/cma/data as a bundle to dev blob projects/ds-seas5-skill/site/<YYYY-MM>/ (pipeline/sync_site_data.py); deploy-pages.yml downloads it (dispatch from the job, or its 10:00 UTC cron on the 7th). Generated data is NOT in git any more."
