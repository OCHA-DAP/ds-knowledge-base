...
discrepancies:
  - "[conflict] CI deploys to Azure SLOTS only, never a production slot. main → slot 'development', sandbox → slot 'sandbox'. No workflow targets a prod slot, so the prod surface (if any) is updated manually / by slot-swap. deployments.md lists only the base app row `chd-ds-data-validation` (Stopped since 2026-08-06) and does not distinguish slots."
