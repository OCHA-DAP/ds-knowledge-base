...
- **URL**: https://chd-ds-floodexposure-monitoring.azurewebsites.net
- **2026-09-25:** Hannah's Carto→OSM basemap fix (6ce3d72) deployed green but the live URL kept the Carto watermark — because of exactly the slot split below. [ds-floodexposure-monitoring-app#62](https://github.com/OCHA-DAP/ds-floodexposure-monitoring-app/pull/62) (pending) adds a `deploy-production` job gated on a manual `deploy_production` input, reusing the 2024 production publish-profile secret. Also verified that day: the production slot, although VNet-integrated, reaches the prod DB over the **public** endpoint (FQDN resolves publicly; no private DNS zone) — see [database.md](../infrastructure/database.md); the `development` and `sudan` slots have no VNet integration at all.
