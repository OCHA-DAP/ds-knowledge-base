...
The internal clone sits next to the public KB clone (`$KB_REPOS_DIR` →
`~/.claude/.kb-repos-dir` → `~/OCHA/repos`); the `kb-access` plugin's sync hook
clones both automatically for users with access.
...
4. **Fallback** when the internal repo isn't on this machine: follow
   `methods/style-guide.md` (public) and tell the user how to get the mirror — it needs
   OCHA-DAP access (`gh auth login`), after which the `kb-access` plugin's next
   session-start sync clones it (KB `docs/USING.md`).
