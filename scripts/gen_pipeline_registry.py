...
def collect_databricks(personal_ids: set) -> list[dict]:
    ...
    for j in jobs:
        ...
        e.update(last_run_dbx(j["job_id"]))   # last-run for every job (prod, dev, trigger/manual)
        out.append(e)
    return out
...
def health(e: dict) -> tuple[str, list[str]]:
    """(status OK|WARN|DOWN|UNKNOWN, flags). Monitors prod AND dev (both scheduled);
    trigger/manual jobs get a light last-run-only check (no cadence).
    ...
    # trigger/manual: no schedule → only judge the last run result (no cadence/overdue).
    if cat == "trigger/manual":
        if str(last) in FAIL_STATES:
            return "DOWN", ["FAILING"]
        return ("OK", []) if last == "SUCCESS" else ("—", [])
    if cat not in ("prod", "dev"):
        return "—", []
    ...
    # MODE / PERSONAL-CLUSTER are only *wrong* for a PROD job — expected for a dev job.
    if cat == "prod" and e["kind"] == "databricks" and e.get("data_mode") and e["data_mode"] != "prod":
        flags.append(f"MODE={e['data_mode']}")
    if cat == "prod" and "personal:" in (e.get("compute") or ""):
...
def main():
    ...
    sort = lambda lst: sorted(lst, key=lambda e: (order[e["_status"]], e["kind"], e["name"]))
    prod = sort([e for e in entries if e["category"] == "prod"])
    dev = sort([e for e in entries if e["category"] == "dev"])
    man = sort([e for e in entries if e["category"] == "trigger/manual"])

    def counts(lst):
        return {s: sum(1 for e in lst if e["_status"] == s) for s in ("DOWN", "WARN", "UNKNOWN", "OK")}
    n, nd = counts(prod), counts(dev)
    ...
    md = [
        ...
        "",
        f"**Prod health: {DOT['DOWN']} {n['DOWN']} down · {DOT['WARN']} {n['WARN']} warn · "
        f"{DOT['UNKNOWN']} {n['UNKNOWN']} unknown · {DOT['OK']} {n['OK']} ok** ({len(prod)} prod) · "
        f"**Dev: {DOT['DOWN']} {nd['DOWN']} · {DOT['WARN']} {nd['WARN']} · {DOT['OK']} {nd['OK']}** ({len(dev)} dev).  ",
        f"Heuristic: OVERDUE = no success within cadence × {GRACE:g}; UNKNOWN = GHA runs not retrievable; "
        "dev jobs are checked the same way but MODE=dev / personal-cluster are expected (not flagged).",
        "",
        "## Prod pipelines",
        "",
        md_table(prod),
        ...
        "(cutover) · **PERSONAL-CLUSTER** prod job pinned to a personal interactive cluster (fragile) "
        ...
        "",
        "Personal / namespaced dev jobs that still run on a schedule — checked the same way as prod, "
        "except `mode=dev` and personal-cluster compute are expected and not flagged.",
        "",
        md_table(dev),
        "",
    ...
    print(f"Wrote {OUT_MD.relative_to(ROOT)} — prod {len(prod)} ({n['DOWN']}↓/{n['WARN']}!/{n['OK']}✓), "
          f"dev {len(dev)} ({nd['DOWN']}↓/{nd['WARN']}!/{nd['OK']}✓), manual {len(man)}.")
    for grp, lbl in ((prod, "prod"), (dev, "dev"), (man, "manual")):
        for e in grp:
            if e["_status"] in ("DOWN", "WARN", "UNKNOWN"):
                print(f"  [{lbl}] {DOT[e['_status']]} {e['name']:34} {', '.join(e['_flags'])}")
