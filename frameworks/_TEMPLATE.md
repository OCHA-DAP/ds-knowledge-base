...
funding_by_sector: {}      # pre-arranged amounts by SECTOR, USD ints, using the doc's own sector labels, e.g. {Nutrition: 6020000, WASH: 300000, "Health & Nutrition": 2000000}. May cover only part of the total (e.g. only the CERF portion is broken down) — partial is fine; say what's covered in extra.funding_breakdown_note. {} if the doc gives no per-sector amounts.
funding_by_agency: {}      # pre-arranged amounts by receiving AGENCY/NGO, USD ints, e.g. {FAO: 3625000, WFP: 980000}. Same rules as funding_by_sector (partial ok, coverage in extra.funding_breakdown_note); envelopes whose partner is not yet selected (e.g. AHF "partner TBD") stay OUT of this dict and are noted instead. Alternative-scenario budgets (only one scenario draws per event) are NOT summed — dicts stay {} with the per-scenario split in the note. {} if not stated.
funding_rows: []           # FINEST-GRAIN budget rows, exactly as the doc states them: [{window, source, agency, sector, amount_usd}] — window/source/agency/sector null (omit) where the doc doesn't split on that axis; sector = doc's own label. Rows are ADDITIVE by definition: sums per axis must reproduce the dicts above over the covered portion. Window-conditional envelopes (same money, sector depends on window) and alternative-scenario budgets stay OUT (combined-label dict entry / note instead). STATED rows only — never impute (the DB loader owns imputation conventions like the 5/95 readiness split). [] if the doc gives no rows beyond the dicts.
...
surfaces: []       # every PUBLISHED URL this framework's repo serves (GH Pages products, Netlify book, Azure app…): [{url, kind, title}] — kind ∈ landing|app|report|book|dashboard|form|download|docs|status|other. The daily pages-registry auto-adds live ones it finds as `auto: true`; you replace `auto` with `kind` on review. (Replaces the retired `apps:` key, D102.) A partner-run tool/document the work relies on (not built by us) gets `origin: external` → the hub's External resources section (D104).
