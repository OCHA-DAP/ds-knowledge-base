...
surfaces: []       # every PUBLISHED URL this framework's repo serves (GH Pages products, Netlify book, Azure app…): [{url, kind, title}] — kind ∈ landing|app|report|book|dashboard|form|download|docs|status|other. The daily pages-registry auto-adds live ones it finds as `auto: true`; you replace `auto` with `kind` on review. (Replaces the retired `apps:` key, D102.) A partner-run tool/document the work relies on (not built by us) gets `origin: external` → the hub's External resources section (D108).
