...
FRAMEWORK vs ANALYSIS: a page is a framework ONLY if a published framework doc exists for THIS specific thing. If there is no framework doc — a regional overview whose components are the real frameworks, an ad-hoc activation, or repo-only exploration that never became an endorsed framework — it is NOT a framework: write an analysis/ page instead (content_type: analysis, per analysis/_TEMPLATE.md), not a frameworks/ page. A real-but-non-public framework doc still counts as a framework (note extra.doc_status).
